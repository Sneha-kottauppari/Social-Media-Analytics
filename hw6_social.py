...
def graphRegionComparison(regionDicts, title):
    ...
    for each in regionDicts:
        ...
        x=regionDicts[each]
        for each in x:
            if each not in featurelst:
                featurelst.append(each)
            templist.append(x[each])
        regionvalue.append(templist)
        regionlst.append(each)
...
def graphHashtagSentimentByFrequency(data):
    ...
    for each in mostcommon:
        hashtaglst.append(each)
        freqlst.append (mostcommon[each])
        sentimentlst.append(getHashtagSentiment(data,each))
